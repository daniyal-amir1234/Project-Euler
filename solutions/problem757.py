...
# "PER" = commented to increase performance

for n in range(1, 10**6): # test the first 100 numbers if they're stealthy
    stealthy_found = False
    for i in range(10**6): # test the first 100 numbers to see if they're factors
        if i != 0: 
            if n % i == 0:
                # print(i, "divides into", n) # PER
                factors.append(i)
    # print(factors, "are the factors of", n) # PER

    for a in range(len(factors)):
        for b in range(len(factors)):
            if (n == factors[a] * factors[b]):
                # print(factors[a], factors[b]) # all pairs here can be ab and/or cd # PER
